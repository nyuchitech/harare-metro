 ...
 * Consolidates frontend SSR + backend admin + auth into one worker
 *
 * Routes:
 * - /                    → React Router SSR (public site)
 * - /admin/*             → Admin panel (requires admin role)
 * - /api/auth/*          → Authentication endpoints (public)
 * - /api/admin/*         → Admin API (requires admin role)
...
// Types for Cloudflare bindings (merged from both workers)
type Bindings = {
  // Database
  DB: D1Database;
...
app.post("/api/auth/register", async (c) => {
  try {
    ...
    const isRateLimited = await rateLimitService.checkRateLimit(rateLimitKey, 3, 3600); // 3 per hour
...
app.post("/api/auth/login", async (c) => {
  try {
    ...
    const isRateLimited = await rateLimitService.checkRateLimit(rateLimitKey, 5, 900); // 5 per 15 minutes
    ...
    // Verify password
    const isValid = await PasswordHashingService.verifyPassword(password, user.password_hash as string);
...
// POST /api/admin/refresh-rss - Manual RSS refresh (requires admin role)
app.post("/api/admin/refresh-rss", requireAdmin, async (c) => {
  try {
    // Implement RSS refresh logic here
    // This will be similar to what was in backend/index.ts
    ...
    console.error('[ADMIN] RSS refresh error:', error);
    return c.json({ error: "RSS refresh failed" }, 500);
  }
});
  ...
  try {
    ...
    return c.json({
      ...
      services: {
        ...
        analytics: !!(c.env.CATEGORY_CLICKS && c.env.NEWS_INTERACTIONS),
...
export default {
  ...
  // Scheduled handler for cron triggers (RSS refresh)
  async scheduled(event: ScheduledEvent, env: Bindings, ctx: ExecutionContext) {
    console.log('[CRON] RSS refresh triggered at', new Date().toISOString());

    // TODO: Implement RSS refresh logic
    // This will use SimpleRSSService and ContentProcessingPipeline
  }
};
