...
export async function loader({ params, request }: Route.LoaderArgs) {
  ...
  try {
    ...
    return { profile, stats, isOwnProfile, token };
  } catch (error) {
...
export default function UserProfile({ loaderData }: Route.ComponentProps) {
  const { profile, stats, isOwnProfile, token } = loaderData;
  ...
  const loadArticles = async (tab: string) => {
    ...
    try {
      const response = await fetch(
        `https://admin.hararemetro.co.zw/api/user/${profile.username}/${tab}?limit=20`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
