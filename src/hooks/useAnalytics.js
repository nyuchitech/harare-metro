...
export function useAnalytics() {
  ...
  const trackEvent = useCallback(async (eventType, data = {}) => {
    try {
      ...
      logger.debug('📊 Analytics tracked:', eventType)
    } catch (error) {
      logger.debug('Analytics failed, using localStorage:', error.message)
  ...
  const syncLocalAnalytics = useCallback(async () => {
    try {
      ...
      if (successCount > 0) {
        ...
        logger.debug(`📊 Synced ${successCount} analytics events`)
      }
    } catch (error) {
      logger.debug('Error syncing local analytics:', error.message)
    }
  }, [trackEvent])
